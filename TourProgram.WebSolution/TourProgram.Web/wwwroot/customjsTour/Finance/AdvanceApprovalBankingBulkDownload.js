    ...
    this.parseExcel = function (file) {
        ...
        reader.onload = function (e) {
            ...
            workbook.SheetNames.forEach(function (sheetName) {
                ...
                tableFinanceBulkApprovalDownLoads.append(`<tr><th>SN</th><th>EmpId</th><th>EmpCode</th><th>Emp Name</th><th>Amount</th><th>SAP Reference</th><th>SAP Ref Date</th></tr>`);
                ...
                for (i = 0; i < productList.length; i++) {
                    ...
                    var trTable = `<tr id="trtbodyDownloadList_${count}">
                                    ...
                                    <input value="${actualRow.id}" type="hidden" id="inputIdHidden_${count}" />
                                       ...
                                       <td>
                                    <span id="tdSapReferenceFromApproval_${count}">${actualRow.sapReference}</span>
                                    <br />
                                    <span id="tdSapReferenceDateFromApproval_${count}">${formatDate_dd_mm_yyyy(new Date(Date.parse(actualRow.sapDate)))}</span >
                                    </td>
                                        <td>
...
function btnUpdate_All(e) {
    ...
    for (var e of $('#tbodyDownloadList > tr')) {
        ...
        var obj = {
            ...
            "sapReferenceBanking": $("#inputSAPReference_" + i).val(),
            "sapDateBanking": formatDate_yyyy_mm_dd(new Date(Date.parse($("#inputSAPDate_" + i).val())))
        }
    ...
    debugger;
    var _url = apiBaseUrl + "​​​​​​/api​/BulkApproval​/UploadBankingBulkApproval​/" + empObject.empId.toString();
    _url = _url.replace(/[\u200B-\u200D\uFEFF]/g, '');
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if ((response.statusCode == 201) || (response.statusCode == 200)) {
                swal({
                    title: 'Success !',
                    text: "Payment details are updated successfully",
                    type: 'success'
                });
            }
            else {
...
function loadDataIntoTable(response) {
    ...
    for(var e of response.data) {
        ...
        var rowApproved = `<tr id="trtbodyDownloadList_${count}">
                                ...
                                <td>
                                    <span id="tdSapReferenceFromApproval_${count}">${e.sapReference}</span>
                                    <br />
                                    <span id="tdSapReferenceDateFromApproval_${count}">${formatDate_dd_mm_yyyy(new Date(Date.parse(e.sapDate)))}</span >
                                </td>
                                <td>
...
function getDataFromApi(loadRequird, callBack) {
    ...
    var url = apiBaseUrl + "​​/api​/BulkApproval​/GetBankingBulkApproval​​/" + empObject.empId.toString();
    url = url.replace(/[\u200B-\u200D\uFEFF]/g, '');
    $.ajax({
        type: "GET",
        url: url,
