...
function bindTableClaimApproved() {
    ...
    debugger;
    var url = apiBaseUrl + "/api/Claim/GetClaimRequest/" + recipientId.toString() + "/1";;
        ...
        success: function (response) {
            ...
            $.each(response.data, function () {
                var rowAdvanceApproved = ` <tr>
                            ...
                            <td>
                                <a onclick="claimed_app(${this.empId},'${this.patientName}','${this.relation}','${this.directClaimId}','${this.employeeName}' ,'${this.patientId}' )" class="btn btn-primary">Details</a>&nbsp;&nbsp;
                            </td>
                        </tr>`;
...
var arrMainBill = [];
var arrMainBillPre = [];
function claimed_app(empId, patientName, relation, directClaimId, empname, pId) {
    $("#divAdvanceApprovedDetails").css('display', 'block');
    AdvanceId = directClaimId;
    var url = apiBaseUrl + "/api/Claim/GetClaimDetails/" + directClaimId;
    url = url.replace(/[\u200B-\u200D\uFEFF]/g, '');
    $.ajax({
        type: "GET",
        url: url,
...
function claimApprovedSubmit_Click(e) {
    ...
    var _url = apiBaseUrl + "​/api​/Claim​/SubmitClaimProcessByHr";
    _url = _url.replace(/[\u200B-\u200D\uFEFF]/g, '');
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if ((response.statusCode == 201) || (response.statusCode == 200)) {
                swal({
                    title: 'Success !',
                    text: "This claim has been successfully forwarded to Doctor for review",
                    type: 'success'
                });
            }
            else {
